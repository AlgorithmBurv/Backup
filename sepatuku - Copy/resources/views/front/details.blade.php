...
<head>
    ...
    <title>Detail Produk - {{ $shoe->name ?? 'AKANTHRA' }}</title>
    ...
    <style>
        /* Custom styles to integrate Poppins font */
        body {
...
<body class="bg-[#F5F5F0]">
    <div class="relative flex flex-col w-full max-w-[640px] min-h-screen gap-5 mx-auto">
                    ...
                    @foreach ($shoe->photos as $itemPhoto)
                        <div class="swiper-slide !w-fit py-[2px]">
                            <label
                                ...
                                <input type="radio" name="image" class="hidden" {{ $loop->first ? 'checked' : '' }}>
        ...
        <form action="{{ route('front.save_order', $shoe->slug) }}" method="POST"
            class="flex flex-col gap-3 pb-[120px]">
            @csrf
                ...
                <div class="flex items-center flex-wrap gap-[10px]">
                    @foreach ($shoe->sizes as $itemSize)
                        <label
                            ...
                            <input type="radio" value="{{ $itemSize->id }}" name="size_id"
            ...
            <div id="form-bottom-nav" class="relative flex w-full shrink-0">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Logic for changing the main image when a thumbnail is clicked
            const mainThumbnail = document.getElementById('main-thumbnail');
            const thumbnailLabels = document.querySelectorAll('.thumbnail-selector');
